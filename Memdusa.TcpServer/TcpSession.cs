...
public class TcpSession : IDisposable
{
    ...
    {
        Server = server;
        _receiveBuffer = server.BufferPool.Rent(BufferSize);
    ...
    protected internal TcpSession Connect(Socket socket)
    {
        ...
        _tasks.Add(Task.Factory.StartNew(DoReceive, _cancellationTokenSource.Token, TaskCreationOptions.LongRunning, TaskScheduler.Default));
        _tasks.Add(Task.Factory.StartNew(SendLoop, _cancellationTokenSource.Token, TaskCreationOptions.LongRunning, TaskScheduler.Default));
    ...
    {
        var buffer = _receiveBuffer.AsMemory();
        ...
        do
        {
            try
            {
                ...
                Socket!.SendBufferSize = Networking.SocketSendBufferSize;
                Socket!.ReceiveBufferSize = Networking.SocketRecvBufferSize;

                // If we are running on linux, set the socket option for quick-ACKs
                ...
                var read = Socket!.Receive(bufferSpan, SocketFlags.None);
                ...
                OnReceived(buffer[..read], _cancellationTokenSource.Token)
        ...
        Log.Debug("Session {SessionId} receive thread stopped!", Id);
        Disconnect();
    }
    ...
    protected virtual Task OnReceived(ReadOnlyMemory<byte> data, CancellationToken cancellationToken)
    {
